  ...
  ({ label, error, helperText, icon, className, ...props }, ref) => {
    return (
      <div className="w-full flex flex-col gap-1.5 text-right">
        ...
        {helperText && !error && <span className="text-xs text-[#66758A]">{helperText}</span>}
      </div>
    );
  }
);
      ...
      <div className="w-full flex flex-col gap-1.5 text-right">
        ...
        <div className="relative flex items-center">
          ...
          <button
            ...
            className="absolute left-3 text-[#66758A] hover:text-[#13233A] focus:outline-none"
          >
...
export const SelectInput: React.FC<SelectProps> = ({ label, error, options, className, ...props }) => {
  return (
    <div className="w-full flex flex-col gap-1.5 text-right">
      ...
      <select
        className={cn(
          "w-full bg-white border border-[#E3EAF3] text-[#13233A] text-sm rounded-xl px-4 py-2.5 outline-none transition-all duration-150 focus:border-[#1769E0] focus:ring-2 focus:ring-[#1769E0]/15",
          error && "border-[#E5484D]",
          className
        )}
        {...props}
